...
class AIService:
    ...
    def lawyer_query(self, user_input: list[str]) -> list[str]:
        ...
        query_emd = query_emd.reshape(1, -1)
        search_emd = np.array(list(self.lawyer_emd.values()))
        print(query_emd.shape)
        print(search_emd.shape)
        cosine_sim = cosine_similarity(
            search_emd,
            query_emd)  # using cosine_sim in scikit learn for quick access
        lawyer_indices, scores = self.get_similiar_lawyers(cosine_sim, 6)
        print(lawyer_indices)
        print(scores)
